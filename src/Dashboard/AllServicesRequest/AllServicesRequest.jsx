...
const AllServicesRequest = () => {
  ...
  const fetchRequests = async () => {
    setLoading(true);
    try {
      const { data } = await axiosPublic.get("/api/requests/admin");
      setRequests(data);
    } catch (error) {
  ...
  const handleDelete = async () => {
    try {
      await axiosPublic.delete(`/api/requests/${deleteId}`);
        ...
        {requests.map((request) => (
          ...
          >
            <div className="flex items-center gap-3 mb-3">
              <img
                src={request.userAvatar || "https://via.placeholder.com/50"}
            ...
            <h4 className="text-lg font-semibold mb-2">{request.requestTitle}</h4>
...
export default AllServicesRequest;
